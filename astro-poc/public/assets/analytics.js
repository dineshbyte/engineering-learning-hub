...
(function () {
    ...
    try {
        if (localStorage.getItem('sd:consent') === 'denied') analyticsConsent = 'denied';
    } catch (e) {
    }
    ...
    function ev(name, params) {
        ...
        if (params) {
            for (var k in params) {
                if (params.hasOwnProperty(k)) base[k] = params[k];
            }
        }
    ...
    function trackFromHref(href) {
        if (!href) return null;
        for (var i = 0; i < SLUGS.length; i++) {
            if (href.indexOf(SLUGS[i] + '/') >= 0) return SLUGS[i];
        }
        return null;
    }
    ...
    var ROUTES = [
        ['a.ritem', function (el) {
            ev('search_result_select', {
                link_text: txt(el.querySelector('.rt') || el),
                result_kind: txt(el.querySelector('.rkind'))
            });
        }],
        ['.nr-sugg button', function (el) {
            ev('search_suggestion_select', {suggestion: txt(el)});
        }],
        ['a.start-here-link', function () {
            ev('browse_vault_click', {});
        }],
        ['.fchip[data-cat]', function (el) {
            ev('filter_select', {filter_category: el.getAttribute('data-cat')});
        }],
        ...
        ['a.pill', function (el) {  // footer cheat-sheet / review-sheet / glossary / resources (card OR preview sheet)
            ...
            ev('resource_click', {
                resource_type: resourceType(href), track: trackFromHref(href),
        ...
        ['a.ghbtn', function (el) {
            ev('github_click', {link_url: el.href});
        }],
        ['a.soclink', function (el) {
            ev('social_click', {link_url: el.href, network: el.getAttribute('aria-label') || txt(el)});
        }],
        ...
        ['#hubbar > a', function (el) {
            ev('back_to_hub', {link_url: el.getAttribute('href')});
        }],
        ['.iqtab[data-tab]', function (el) {
            ev('interview_tab_select', {level: el.getAttribute('data-tab')});
        }],
        ['.ltags .lt', function (el) {
            ev('tag_chip_click', {tag: txt(el)});
        }],
        ['sup a[href^="#"]', function (el) {
            ev('footnote_click', {anchor: el.getAttribute('href')});
        }],
            ...
            if (!localStorage.getItem('sd:consent')) {
                var mount = function () {
                    ...
                    var decide = function (v) {
                        try {
                            localStorage.setItem('sd:consent', v);
                        } catch (e) {
                        }
                    ...
                    b.querySelector('#sdc-yes').onclick = function () {
                        decide('granted');
                    };
                    b.querySelector('#sdc-no').onclick = function () {
                        decide('denied');
                    };
                };
    ...
    if (CTX.page_type === 'lesson') {
        ...
        function markDepth(p) {
            ...
            [25, 50, 75, 100].forEach(function (m) {
                if (!depthSent[m] && pct >= m) {
                    depthSent[m] = true;
                    ev('scroll_depth', {percent: m, lesson_number: CTX.lesson_number});
                }
            });
        }
