...
Column stores flip the layout: store all values of *one column* contiguously, in its own file or block, rather than all values of one row. The idea is old — it traces to the C-Store research prototype (Stonebraker et al., "C-Store: A Column-oriented DBMS", VLDB 2005), which became the commercial Vertica — and Google's Dremel (Melendez et al., "Dremel: Interactive Analysis of Web-Scale Datasets", VLDB 2010) brought it to nested, web-scale data; Dremel's storage format is the ancestor of Apache Parquet. DDIA ch.3 covers the design under "Column-Oriented Storage."
